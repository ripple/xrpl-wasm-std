async function test(testContext) {
  try {
    const { submit, sourceWallet, offerSequence, allWallets } = testContext
    ...
    const finishWallet = allWallets[3]
    const txFail = {
      TransactionType: "EscrowFinish",
      Account: finishWallet.address,
      Owner: sourceWallet.address,
      OfferSequence: parseInt(offerSequence),
      ComputationAllowance: 1000000,
    }

    const responseFail = await submit(txFail, finishWallet)
