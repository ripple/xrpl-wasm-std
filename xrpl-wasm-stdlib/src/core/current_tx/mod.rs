...
/// Implementation of `CurrentTxFieldGetter` for Vector256 (array of 256-bit hashes).
///
/// This implementation handles Vector256 fields in XRPL transactions, which are arrays
/// of 32-byte hashes. Common uses include CredentialIDs, NFTokenOffers, and other fields
/// that contain multiple hash values.
///
...
/// # Buffer Management
///
/// Uses a 257-byte buffer (1 byte for length + 8 × 32 bytes for hashes) to accommodate
/// the maximum supported vector size. The actual data is parsed by Vector256::from_bytes().
///
/// # Example
///
/// ```no_run
...
/// use xrpl_wasm_stdlib::sfield;
///
/// // Get CredentialIDs from the current transaction
/// let credential_ids: Option<Vector256> = get_field_optional(sfield::CredentialIDs).unwrap();
/// if let Some(creds) = credential_ids {
...
/// }
/// ```
impl CurrentTxFieldGetter for Vector256 {
    #[inline]
    fn get_from_current_tx(field_code: i32) -> Result<Self> {
...
mod tests {
    ...
    #[test]
    fn test_vector256_from_bytes_empty() {
    ...
    fn test_vector256_from_bytes_single_hash() {
        ...
        for i in 0..32 {
            assert_eq!(hash.as_bytes()[i], (i as u8).wrapping_mul(17));
        }
    }

    #[test]
    fn test_vector256_from_bytes_multiple_hashes() {
        let mut bytes = [0u8; 97]; // 1 + 3*32 = 97 bytes
        bytes[0] = 96; // data length = 96 (3 hashes × 32 bytes)
        ...
        assert_eq!(vec.get(0).unwrap().as_bytes()[0], 0xAA);
        assert_eq!(vec.get(1).unwrap().as_bytes()[0], 0xBB);
        assert_eq!(vec.get(2).unwrap().as_bytes()[0], 0xCC);
        assert!(vec.get(3).is_none());
    }

    #[test]
    fn test_vector256_from_bytes_max_size() {
        // Note: MAX_VECTOR256_SIZE * 32 = 256, which wraps to 0 in u8
        // So we test with MAX_VECTOR256_SIZE - 1 instead
        let test_size = MAX_VECTOR_256_SIZE - 1;
        let mut bytes = [0u8; 1 + ((MAX_VECTOR_256_SIZE - 1) * 32)];
        bytes[0] = (test_size * 32) as u8; // data length = 7 × 32 = 224

        // Fill each hash with its index
        for i in 0..test_size {
            for j in 0..32 {
                bytes[1 + (i * 32) + j] = i as u8;
            }
        }

        let vec = Vector256::from_bytes(&bytes).unwrap();
        assert_eq!(vec.len(), test_size);

        for i in 0..test_size {
            let hash = vec.get(i).unwrap();
            assert_eq!(hash.as_bytes()[0], i as u8);
        }
    }

    #[test]
    fn test_vector256_from_bytes_buffer_too_small() {
        let bytes = [64u8, 0, 0]; // Says 64 bytes but only has 2 bytes of data
    ...
    #[test]
    fn test_vector256_from_bytes_empty_buffer() {
        let bytes = [];
        let result = Vector256::from_bytes(&bytes);
        assert!(result.is_err());
        match result {
    ...
    #[test]
    fn test_vector256_from_bytes_count_too_large() {
        let mut bytes = [0u8; 1];
        // Test with a data length that's not a multiple of 32
        bytes[0] = 33; // Not a multiple of 32, should return InvalidParams
        let result = Vector256::from_bytes(&bytes);
        assert!(result.is_err());
        match result {
    ...
    #[test]
    fn test_vector256_to_bytes_empty() {
        let vec = Vector256::new();
        let (bytes, len) = vec.to_bytes();
        assert_eq!(len, 1);
        assert_eq!(bytes[0], 0); // data length = 0
    }

    #[test]
    fn test_vector256_to_bytes_single() {
        let mut vec = Vector256::new();
        vec.len = 1;
        vec.hashes[0] = Hash256::from([0x42; 32]);

        let (bytes, len) = vec.to_bytes();
        assert_eq!(len, 33); // 1 + 32
        assert_eq!(bytes[0], 32); // data length = 32 (1 × 32)
        assert_eq!(bytes[1], 0x42); // first byte of hash
        assert_eq!(bytes[32], 0x42); // last byte of hash
    }

    #[test]
    fn test_vector256_to_bytes_roundtrip() {
        let mut vec = Vector256::new();
        vec.len = 2;
        ...
        let (bytes, len) = vec.to_bytes();
        let vec2 = Vector256::from_bytes(&bytes[..len]).unwrap();

        assert_eq!(vec, vec2);
    }

    #[test]
    fn test_vector256_as_slice() {
        let mut vec = Vector256::new();
        vec.len = 3;
        vec.hashes[0] = Hash256::from([0x11; 32]);
        vec.hashes[1] = Hash256::from([0x22; 32]);
        vec.hashes[2] = Hash256::from([0x33; 32]);

        let slice = vec.as_slice();
        assert_eq!(slice.len(), 3);
        assert_eq!(slice[0].as_bytes()[0], 0x11);
        assert_eq!(slice[1].as_bytes()[0], 0x22);
        assert_eq!(slice[2].as_bytes()[0], 0x33);
    }
}
