...
class AccountSyncClass {
  ...
  sync() {
    ...
    console.warn('AccountSync: sync() called but getAccounts not set. Call autoSync() first.')
    return false
  }
  ...
  removeAccount(index) {
    if (!this._getAccounts || !this._setAccounts) {
      console.warn('AccountSync: removeAccount() called but autoSync not enabled')
      return false
    }
